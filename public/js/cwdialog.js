...
function CWdialog(text, options) {
  ...
  $.each(settings.buttons, function (i, button) {
    if (button) {
      // Create button
      var $_button = $('<button id="cwdialog-btn-' + button.id + '">').append(
        button.text
      );
  ...
  $CWdialog
    .html("")
    .append($_inner.append('<div class="cwdialog-content">' + text + "</div>"))
    .append($_buttons);
  $cA = this;

  if (settings.input) {
    $_inner
      .find(".cwdialog-content")
      .append($('<div class="cwdialog-input">').append($_input));
  }
